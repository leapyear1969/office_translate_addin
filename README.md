...
- “恢复原文”需在面板确认，会替换整份正文，撤回备份之后的所有正文翻译及编辑，即使之前只翻译了选区或段落。页眉、页脚等独立内容不在备份与恢复范围内。这是一份固定原文备份，并非逐次翻译历史。恢复后不会自动再次翻译。
- 范围在登录和请求前捕获并跟踪，写回前重新读取并精确比较范围文字（包括空白）；文字已修改则取消替换。HTML/OOXML 导出字符串不作为内容版本号，避免导出差异触发误报。此检查不检测仅格式、图片等非文字变化，翻译等待期间请避免这些编辑。翻译使用 HTML，恢复使用备份的 OOXML；不能保证复杂 Word 格式、域、内容控件或修订的完整保真，正式使用前用代表性文档验收。
