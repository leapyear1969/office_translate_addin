...
- “恢复原文”确认后逐个恢复未被修改的翻译范围，保留其他位置的编辑。译文文字或 HTML 格式与翻译后快照不一致、定位标记重复、备份未完成时跳过并提示；定位标记已删除时不恢复。旧版整篇备份保留，但禁止用它覆盖正文。已恢复的记录保留，以支持 Word 撤销恢复；恢复后重新编辑并翻译会生成新的范围备份。重叠范围需先恢复再翻译。全文翻译作为一个范围，范围内有编辑时整次跳过。
- 范围在登录和请求前捕获并跟踪，写回前精确比较文字（包括空白），文字变化时取消替换；不将 HTML 导出结果当作版本号，避免保存备份引起的导出差异误报。此翻译前检查不检测仅格式或图片变化，翻译等待期间请避免这些编辑。恢复也采用保守校验，Word 导出或重新打开导致的 HTML 序列化差异可能使未编辑内容被跳过，不会因此强行覆盖。翻译使用 HTML，恢复使用 OOXML；复杂格式、图片、域、内容控件和修订仍需真实 Word 验收。
