...
- 范围在登录和请求前捕获并跟踪，写回前精确比较文字（包括空白），文字变化时取消替换；不将 HTML 导出结果当作版本号，避免保存备份引起的导出差异误报。此翻译前检查不检测仅格式或图片变化，翻译等待期间请避免这些编辑。恢复精确检查译文文字，不比较 HTML 导出字符串；恢复会还原该范围的原文及原格式，因此范围内后改的格式、图片等非文字内容也会还原。范围外的编辑保持不变。翻译使用 HTML，恢复使用 OOXML；复杂格式、图片、域、内容控件和修订仍需真实 Word 验收。
...
### Word 网页版 OOXML 导出兼容
- 当选区原文 `getOoxml()` 返回 `ooxmlIsMalformed` / `ooxmlIsMalformated` / `InvalidOoxml` 时，改用已读取的该范围 HTML 保存原文。其他导出错误仍中止，不对写入错误自动重试。
...
- 翻译先将 HTML 插入原范围，再对 `insertHtml` 返回的实际译文范围创建内容控件，避免依赖原段落边界。创建和设置标记分开同步，错误显示具体 API 位置。
