...
通过环境变量 `ANALYTICS_RETENTION_MONTHS` 配置日汇总保留的自然月数（整数 1–1200，未设置或为空默认 12），修改后重启后端。启动及每隔 24 小时清理早于保留起始日的日汇总，按北京时间计算并保留起始日当天；写入与查询日期限制使用相同配置。缩短期限会删除旧日汇总，延长期限不会恢复已删除数据。普通清理保留用户首次时间。请求未结束或进程崩溃时可能未计入；已知缺口在后续数据库可写时保存。采集前、已清理或已丢失的历史不显示为确认的零用量。
