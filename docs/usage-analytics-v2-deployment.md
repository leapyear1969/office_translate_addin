...
- 管理员租户：`d7125684-0e28-40b5-aba2-ea9580f2a201`；邮箱：`jason@majun.fun`；本地配置已设为 `tenants:["*"]`，可查询所有现有及未来新增租户；线上需同步本次代码、配置并重启后生效。
...
通过环境变量 `ANALYTICS_RETENTION_MONTHS` 配置日汇总保留的自然月数（整数 0–1200，0 表示永久保留，未设置或为空默认 12），修改后重启后端。设置为 0 时跳过按保留期清理，不限制历史写入与查询的最早日期，后台显示“日汇总永久保留”，默认查询仍为最近 30 天。设置为 1–1200 时，启动及每隔 24 小时清理早于保留起始日的日汇总，按北京时间计算并保留起始日当天；写入与查询日期限制使用相同配置。缩短期限或从永久保留改为有限期限会删除超期日汇总；延长期限或改为永久保留不会恢复已删除数据。普通清理保留用户首次时间。请求未结束或进程崩溃时可能未计入；已知缺口在后续数据库可写时保存。采集前、已清理或已丢失的历史不显示为确认的零用量。
