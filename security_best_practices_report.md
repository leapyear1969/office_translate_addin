...
- 第 4 项：后台路由与 `/admin.html`（含编码静态别名）统一返回 CSP、`frame-ancestors 'none'`、`X-Frame-Options: DENY` 和 `nosniff`，继续禁用缓存和 Referrer。脚本、样式只允许同源，连接仅允许同源 API、配置的 authority 及 MSAL 所需的发现/中国云端点。Office 插件页面不增加嵌入限制。MSAL 采用 `AccessTokenAndRefreshToken`，保留刷新令牌续期，过期后点击登录按钮重新登录，不再尝试隐藏 iframe。
- 第 5 项：通过 overrides 将 `qs` 固定为 6.16.0、MSAL Node 下的 `uuid` 固定为 11.1.1，并更新 lockfile；保持 Express 4 和 MSAL Node 3。UUID 覆盖跨越了上游声明范围，已验证实际 MSAL CommonJS 加载与 v4 关联 ID 生成，以及 uuid/qs 公告边界用例；后续上游原生采用修复版本后应移除覆盖。

本次验证：33 个套件、482 项测试全部通过；`npm run typecheck`、`npm run build` 通过；官方 registry 的 `npm audit --omit=dev` 为 0。全依赖审计仍有 6 个开发工具链受影响包条目（4 moderate、2 high，涉及 Office manifest 工具和 webpack-dev-server 等），不计入本报告第 5 项原定的生产依赖范围。没有验证生产安装版本或使用真实 Entra 账号验收登录；部署及浏览器验收步骤见 [后台说明](docs/admin-settings.md)。下文保留原审查证据。
...
第 1–3 项当时验证：针对性测试 8 个套件、63 项通过；随后全量 `npm test -- --runInBand` 的 31 个套件、467 项测试全部通过；`npm run typecheck` 通过。验证覆盖邮箱复用、旧条目迁移、匿名/无效令牌限流、不同账号/租户配额、增删改审计、操作者伪造、版本冲突、审计失败回滚和追加记录保护。未执行生产数据库迁移或线上角色验收。第 4、5 项的后续修复见上。
