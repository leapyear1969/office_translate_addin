...
async function start() {
  ...
  if (process.env.NODE_ENV === 'production' && !process.env.SSL_CERT_PATH) {
    app.listen(config.port, '127.0.0.1', () => console.log(`Backend listening on 127.0.0.1:${config.port}; HTTPS reverse proxy required.`));
    return;
  }
  ...
  https.createServer(certificate, app)
    ...
    .listen(config.port, 'localhost', () => console.log(`Outlook translation available at ${config.origin}`));
}
