...
function render(data: Row) {
  ...
  if (isApi) {
    ...
    table('interfaces', ['层级', '接口', '请求数', '成功', '失败', '提交文本量（UTF-16）'], data.interfaces.map((r: Row) => [label(r.layer), r.endpoint, r.requests, r.successes, r.failures, r.layer === 'upstream' ? r.submitted_units : '仅在上游统计']), [2, 3, 4, 5]);
